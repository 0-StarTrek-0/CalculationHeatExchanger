...
#include "ui_tubeontube.h"

#include <QVariant>
#include <QAxObject>

TubeonTube::TubeonTube(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::TubeonTube)
{
    ui->setupUi(this);
    control();
    //connect()
...
TubeonTube::~TubeonTube()
{
    delete ui;
}

///////////////////////////////Меню///////////////////////////////////////////
void TubeonTube::on_Exit_menu_triggered()
{
    QApplication::quit();
}

void TubeonTube::on_Open_menu_triggered()
{
    QString File;
    ...
    if(outFile.open(QIODevice::ReadOnly| QIODevice::Text) ) {
        ...
        ui->line_d->setText(stream.readLine());
        ui->line_D->setText(stream.readLine());
...
void TubeonTube::on_Save_menu_triggered()
{
    if (NameFile == "" or NameFile == " ")
        NameFile = QFileDialog::getSaveFileName(this, "Сохранить в файл...", "C:\\Users\\Ivan\\Desktop", "Текстовый документ (*.txt);");
    QFile inFile(NameFile);
...
void TubeonTube::on_Save_as_menu_triggered()
{
    QString File;
...
void TubeonTube::on_Clean_menu_triggered()
{
    //Горячий контур
...
void TubeonTube::on_Create_menu_triggered()
{
    NameFile = QFileDialog::getSaveFileName(this, "Сохранить в файл...", "C:\\Users\\Ivan\\Desktop", "Текстовый документ (*.txt);");
    //Горячий контур
    ui->line_hot_T1->clear();
...
void TubeonTube::on_Back_menu_triggered()
{
    ui->statusbar->showMessage("Выход в главное меню");
}

void TubeonTube::on_Start_menu_triggered()
{
    on_Button_Start_clicked();
}

void TubeonTube::on_Check_menu_triggered()
{
...
void TubeonTube::on_T_ex_cold_menu_triggered()
{
    if (ui->line_hot_T1->text() != "" &&
        ui->line_hot_T2->text() != "" &&
        ui->line_hot_pressure->text() != "" &&
...
void TubeonTube::on_T_ex_hot_menu_triggered()
{
    if (ui->line_hot_T1->text() != "" &&
        ui->line_hot_pressure->text() != "" &&
        ui->line_hot_dmass->text() != "" &&
...
void TubeonTube::control()
{
    ...
    ui->line_d->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9][0-9]*\\.[0-9][0-9][0-9][0-9]"), this));
    ui->line_D->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9][0-9]*\\.[0-9][0-9][0-9][0-9]"), this));
    ui->line_delt_wall->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9][0-9]*\\.[0-9][0-9][0-9][0-9]"), this));
}

/////////////////////////////////Кнопки/////////////////////////////////////
void TubeonTube::on_Button_save_exel_clicked()
{
    try {
        QString NameExcel;
...
void TubeonTube::on_Button_clear_table_clicked()
{
    for(int row = 0; row < 18; row++) //String
    {
        QTableWidgetItem *item = new QTableWidgetItem();
...
void TubeonTube::on_Button_Exit_clicked()
{
    QApplication::quit();
}

void TubeonTube::on_Button_Start_clicked()
{
    ...
    {        
        ...
        if (D<=d+delt)
        {
            ui->statusbar->showMessage("Введите корректные данные... Внешняя труба не может быть меньше внутренней!");
            return;
        }
...
void TubeonTube::on_cBox_cavity_hot_currentIndexChanged(int index)
{
    if ((ui->cBox_cavity_hot->currentIndex() == 0 and ui->cBox_cavity_cold->currentIndex() == 1)
        or (ui->cBox_cavity_hot->currentIndex() == 1 and ui->cBox_cavity_cold->currentIndex() == 0))
    {
        ui->statusbar->showMessage("Ошибка расчета: пересечение областей расчета");
        ui->cBox_cavity_hot->setCurrentIndex(index);
        ui->cBox_cavity_cold->setCurrentIndex(index);
    }
}

void TubeonTube::on_cBox_cavity_cold_currentIndexChanged(int index)
{
    if ((ui->cBox_cavity_hot->currentIndex() == 0 and ui->cBox_cavity_cold->currentIndex() == 1)
        or (ui->cBox_cavity_hot->currentIndex() == 1 and ui->cBox_cavity_cold->currentIndex() == 0))
    {
